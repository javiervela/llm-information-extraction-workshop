...
class BatchAnalyzer:
    ...
    def __init__(self, model_name: str = "llama3.1", ollama_url: str = "http://localhost:11434", max_workers: int = 2):
        self.model_name = model_name
        self.ollama_url = ollama_url
        self.max_workers = max_workers
        self.results = []
        
    def find_text_files(self, directory: str, extensions: List[str] = ['.txt', '.md']) -> List[Path]:
    ...
    def analyze_single_file(self, file_path: Path, analysis_type: str = "general") -> Dict:
        ...
        try:
            ...
            # Generar reporte
            output_file = file_path.parent / f"{file_path.stem}_{analysis_type}_report.md"
    ...
    def process_files(self, files: List[Path], analysis_type: str = "general") -> List[Dict]:
        ...
        with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            # Enviar tareas
            future_to_file = {
                executor.submit(self.analyze_single_file, file_path, analysis_type): file_path 
            ...
            for future in as_completed(future_to_file):
                ...
                try:
                    ...
                    print(f"❌ [{completed+1}/{len(files)}] {file_path.name} - Error: {e}")
                    completed += 1
        
        elapsed_time = time.time() - start_time
        print("-" * 60)
        print(f"⏱️  Tiempo total: {elapsed_time:.2f} segundos")
        print(f"📈 Archivos procesados: {len([r for r in results if r['status'] == 'success'])}/{len(files)}")
    ...
    def generate_batch_summary(self, results: List[Dict], output_file: str):
        ...
        avg_keywords = sum(r.get('keywords_count', 0) for r in successful) / len(successful) if successful else 0
        sentiment_counts = {}
        for r in successful:
...
def main():
    parser = argparse.ArgumentParser(description="Análisis en lote de textos largos con Ollama")
    parser.add_argument("directory", help="Directorio que contiene los archivos a analizar")
    parser.add_argument("-t", "--type", choices=["general", "interview"], default="general",
                        help="Tipo de análisis (general o interview)")
    parser.add_argument("-m", "--model", default="llama3.1", help="Modelo de Ollama a usar")
    parser.add_argument("-w", "--workers", type=int, default=2, help="Número de workers paralelos")
    ...
    if not files:
        print(f"❌ No se encontraron archivos con extensiones {args.extensions} en {args.directory}")
        sys.exit(1)
