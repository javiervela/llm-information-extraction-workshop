...
class InterviewAnalyzer(LongTextAnalyzer):
    ...
    def extract_main_insights(self, text: str) -> List[str]:
        ...
        response = self._call_ollama(prompt, system_prompt)
        insights = [line.strip().lstrip('- •*1234567890.') for line in response.split('\n') if line.strip()]
        return insights[:7]
    ...
    def extract_highlight_quotes(self, text: str) -> List[str]:
        ...
        response = self._call_ollama(prompt, system_prompt)
        quotes = [line.strip().strip('"').strip("'") for line in response.split('\n') if line.strip() and ('"' in line or "'" in line)]
        return quotes[:8]
    
    def analyze_question_themes(self, text: str) -> List[str]:
        """Analiza los temas de las preguntas realizadas"""
        system_prompt = "Eres un experto en análisis de entrevistas y técnicas de interrogación."
        ...
        response = self._call_ollama(prompt, system_prompt)
        themes = [line.strip().lstrip('- •*1234567890.') for line in response.split('\n') if line.strip()]
        return themes[:6]
    ...
    def analyze_interaction_style(self, text: str) -> str:
        ...
        system_prompt = "Eres un experto en análisis de comunicación y dinámicas interpersonales."
...
def main():
    parser = argparse.ArgumentParser(description="Analizador especializado de entrevistas con Ollama")
